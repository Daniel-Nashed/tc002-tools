...
component in this project cross-compiles FOR the TC002 device in [build/docker-alpine-arm](../docker-alpine-arm/README.md)'s
ARM musl image; `tc002-discover` instead runs ON whatever machine is doing device discovery, so it needs a plain
...
the ARM cross-compile image would mean either installing an unrelated second toolchain into every other component's
build environment, or adding an image-selection flag to `build/docker-alpine-arm/run.sh` that every other script would
have to ignore. A separate, independent image keeps the two pipelines apart.
