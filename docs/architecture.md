...
build_curl.sh, ...     execs build/docker/run.sh build/build_<name>.sh. One
...
test_build_nshbox_x86.sh
                       Local x86 dev-only nshbox test build - deliberately
...
build/docker/          Disposable Debian Buster ARMHF cross-build container
                       image (recommended over the bare-host setup script -
                       see build_platform.md). Used for every component
                       except tc002-discover.
...
build/work/            Disposable source extraction/object files from a
                       build - gitignored, safe to delete.
...
tests/                 Build-artifact tests (run inside the container, see
                       ...
                       tools, run inside its own container.
...
A static glibc build was tried and rejected: it produced NSS-related warnings for passwd/group/shadow/resolver functions
and was significantly larger than the dynamic build. The dynamically linked build runs cleanly on the tested device. A
static musl build remains a possible future portability option but is not required for the verified rollout - see
[build_platform.md](build_platform.md) for what to check before trusting the dynamic build against a different firmware
image.
...
- Dynamically linked, stripped Dropbear (`dropbear`, `scp`, `dropbearkey`) runs on-device.
...
- `build/build_dropbear.sh`, `build/build_nshbox.sh`, `build/build_kilo.sh`, and `build/build_ncdu.sh` all
  ...
  confirmed ARM 32-bit hard-float, dynamically linked (statically for `ncdu`'s `ncursesw`/`tinfo`), and stripped.
