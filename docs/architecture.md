...
| `docker-alpine-arm/build-image.sh` | host             | The image build itself: tags the result with the inputs tag and `:latest`, keeps the full output in `build/work-musl/image-build.log`. Called by `run.sh` and `build_image.sh`. |
